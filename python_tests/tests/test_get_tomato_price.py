...
def test_get_tomato_price_positive(api_context, tomato_type):
    response = api_context.get_tomato_price(tomato_type)
    Asserts.assert_status_code(response, 200)

@pytest.mark.parametrize('tomato_type', [NegativeData.INVALID_TOMATO_TYPE])
def test_get_tomato_price_negative(api_context, tomato_type):
    response = api_context.get_tomato_price(tomato_type)
